...
namespace bytedance_db_project
{
  ...
  int32_t CustomTable::GetIntField(int32_t row_id, int32_t col_id)
  {
    ...
    int32_t result;
    char *temp = new char[FIXED_FIELD_LEN];
    memcpy(temp, columns_ + col_id * num_rows_ * FIXED_FIELD_LEN + row_id * FIXED_FIELD_LEN, FIXED_FIELD_LEN);
                                           ...
                                           int32_t threshold2)
  {
    ...
    std::set_intersection(row1.begin(), row1.end(),
                          row2.begin(), row2.end(),
